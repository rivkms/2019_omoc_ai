...
class Board(object):
    ...
    def winner(self):
        ...
        if score[B] > score[W]+1: return B
        else: return W  # 1.5-point compensation (Komi) for W
    ...
    def hashValue(self):
        ...
        for i in range(N):
            for j in range(N):
                ...
                sum *= 3
        sum //= 3
        if self.ko[0] == self.ko[1] == None:
            offset = N*N
        else:
            offset = self.ko[0]*N + self.ko[1]
        return sum*(N*N+1) + offset

    # used by Minimax/AlphaBeta search
